...
function extractLatencyMetrics(payload: ElevenLabsWebhookPayload): LatencyMetrics | null {
    try {
        ...
        // Calculate TTFB (first agent response with LLM metrics)
        const firstAgentTurnWithLLM = agentTurns.find(turn => 
            turn.conversation_turn_metrics?.metrics?.convai_llm_service_ttfb?.elapsed_time !== undefined
        );
        const ttfbMs = firstAgentTurnWithLLM?.conversation_turn_metrics?.metrics?.convai_llm_service_ttfb?.elapsed_time !== undefined
            ? firstAgentTurnWithLLM.conversation_turn_metrics.metrics.convai_llm_service_ttfb.elapsed_time * 1000
            : null;
        ...
        return {
            ...
            tts_ttfb_ms: avgTtsTtfb !== null ? Number(avgTtsTtfb.toFixed(2)) : null,
        ...
        if (latencyMetrics) {
            ...
            const { error: latencyError } = await supabase
                ...
                .insert({
                    ...
                    tts_ttfb_ms: latencyMetrics.tts_ttfb_ms,
