...
class RestaurantAgent(Agent):
    ...
    @function_tool()
    async def add_to_order(self, item_name: str, quantity: int = 1, special_instructions: str = "") -> str:
        """
        ...
        # Keep response brief - don't repeat price every time
        if quantity == 1:
            response = f"Got it, {item['item_name']}."
        else:
            response = f"Got it, {quantity} {item['item_name']}."
        if special_instructions:
            response += f" With {special_instructions}."
    ...
    async def get_order_summary(self, confirm: bool = True) -> str:
        ...
        for item in self.current_order:
            ...
            if item["quantity"] == 1:
                ...
                items.append(f"{item['quantity']} {item['name']}")
            total += subtotal
    ...
    @function_tool()
    async def submit_order(self, customer_name: str, customer_phone: str, order_type: str = "pickup") -> str:
        """
        ...
        order_data = {
            ...
            "special_instructions": "Voice order: " + ", ".join(f"{i['quantity']}x {i['name']}" for i in self.current_order),
        ...
        item_count = sum(i["quantity"] for i in self.current_order)
        wait_time = 15 + min(item_count * 2, 10)

        # Clear order
        self.current_order = []

        # Concise confirmation with goodbye
        return (
            f"Perfect {customer_name}, your order is in! "
            f"It'll be ready for {order_type} in about {wait_time} minutes. "
